...
export default function Dashboard() {
  ...
  const priorityGroups = [
    ...
    ["obc_certificate", "ews_certificate", "obc_ews_certificate"],
  ...
  const matchesPriority = (serviceType: string, candidates: string[]) => {
    const normalizedType = normalizeServiceType(serviceType);
    return candidates.some((candidate) => normalizedType === candidate || normalizedType.includes(candidate));
  };

  const validServices = services.filter(
    (service) => isNonEmpty(service.service_name) && isNonEmpty(service.service_type)
  );

  const consumedIndexes = new Set<number>();
  const prioritizedServices: ServiceSchema[] = [];

  priorityGroups.forEach((group) => {
    validServices.forEach((service, index) => {
      if (consumedIndexes.has(index)) return;
      if (matchesPriority(service.service_type, group)) {
        prioritizedServices.push(service);
        consumedIndexes.add(index);
      }
    });
  });

  const remainingServices = validServices.filter((_, index) => !consumedIndexes.has(index));
  const orderedServices = [...prioritizedServices, ...remainingServices];
    ...
    <div className="services-page">
      ...
      <ChatWidget
        ...
        context={{
          ...
          services: orderedServices.map((svc) => ({
